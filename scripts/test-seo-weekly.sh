...
# Weekly KPI Bot API のテストスクリプト
# 使用方法:
#   ./scripts/test-seo-weekly.sh [CRON_SECRET] [--dry-run]
#   export CRON_SECRET=your-secret-token
#   ./scripts/test-seo-weekly.sh --dry-run

set -e

API_URL="https://mi-business-online.onrender.com/api/cron/seo-weekly"
TOKEN=""
DRY_RUN=false
...
if [ -z "$CRON_SECRET" ]; then
  ...
  echo ""
  echo "使用方法:"
  echo "  ./scripts/test-seo-weekly.sh your-secret-token"
  echo "  ./scripts/test-seo-weekly.sh your-secret-token --dry-run"
  echo "  または"
  echo "  export CRON_SECRET=your-secret-token"
  echo "  ./scripts/test-seo-weekly.sh --dry-run"
  exit 1
fi
...
echo "1. Weekly KPI Bot APIを呼び出し中..."
...
if [ "$HTTP_STATUS" = "200" ]; then
  ...
  if [ "$SUCCESS" = "true" ]; then
    echo "✅ 成功: Weekly KPI Bot APIが正常に呼び出されました"
  else
