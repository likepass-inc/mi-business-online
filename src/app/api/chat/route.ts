...
1. 「現状の要約」セクションに「HTML分析（コンテンツSEO）」のサブセクションを必ず追加してください：
   ...
   - カノニカルURL: ${scrapedData.hasCanonical ? scrapedData.canonicalUrl || '設定済み' : '未設定'}
        ...
        2. 「改善提案」セクションの各提案に、必ずBefore/After形式を含めてください：
           ...
           - 【重要】文字数のカウントは、実際の文字列を1文字ずつ数えて正確に行ってください。改行、スペース、全角/半角の違いも含めて正確にカウントしてください。文字数を記載する際は、必ず実際の文字列の文字数を正確に数えてから記載してください。
           - 変更理由: なぜその変更が必要かの説明
...
1. タイトルタグ分析
   ...
   - 【重要】改善案の文字数を記載する際は、提案する文字列を実際に1文字ずつ数えて正確にカウントしてください。改行、スペース、全角/半角の違いも含めて正確にカウントしてください。
...
- 【必須】Before/After形式で文字数を記載する際は、実際の文字列を1文字ずつ数えて正確にカウントしてください。特に「After」の文字数は、提案する文字列を実際に数えてから記載してください。改行、スペース、全角/半角の違いも含めて正確にカウントしてください。
- 優先度（高/中/低）
